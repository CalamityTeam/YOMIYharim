...
const special = ["Catbox","Ozzatron","Dia","Death","Script","Mana","NightDev","Cherry","triple","Nezriani","Hayabo-of-the-so","Videofreak","Sugar_Father","Kanavaer","HellYeah!!!","Trinity","spooderp","IppoCooking", "player_sans9", "playersans21", "Draedon", "nill bye"]
var charname = "Yharim"
...
onready var INSTALLBGLORDE = preload("res://Yharim/Characters/Yharim/Fx/YharimBGLORDE.tscn")
...
func is_fight_Catbox():
	if not SteamLobby.SPECTATING:
		var users = Network.pid_to_username(opponent.id)
		return users == "Catbox" or users == "Ozzatron"
	return false

func is_Script():
	if not SteamLobby.SPECTATING:
		var users = Network.pid_to_username(id)
		return users == "Script"
	return false
...
# and yes, if you did this for the skin name, you name a style "LORDE"
	...
	if style != null and !is_ghost and style.style_name == "AuricStorm" and is_Special():
		SKIN = true 
	if style != null and !is_ghost and style.style_name == "touhou" and is_Script():
		touhou = true
	if style != null and !is_ghost and style.style_name == "nill" and is_nill():
		nill = true
...
func tick():
	...
	# Code to stop music stacking
	if getOpponentName() == "Yharim":
		if opponent.musicplaying:
			musicplaying = true
	...
	if turn_frames == 1 && !blockstun_ticks:
		if reducebutton:
			snap -= 24
			charge -= 1
	# Lorde Death SFX
	...
	if time < 40 and timer == false:
		time = 40
	# this is what lets you use all the cool stuff anytime you want in inf resourses or ex turbo mode
	...
	if CT == 0:
		...
		Overcharge = false
	# Achievement stuff
	if visible_combo_count >= 10:
		var codex_lib = get_node_or_null("/root/CharCodexLibrary")
		if is_instance_valid(codex_lib):
			codex_lib.unlock_achievement(self, "combo10", true)
	...
	if win == false and opponent.hp <= 0:
		...
		if is_instance_valid(codex_lib):
			...
			if codex_lib.achievement_target_met(self, "Win502"):
				codex_lib.unlock_achievement(self, "Win502", true)
			win = true
