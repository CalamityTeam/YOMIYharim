...
func _frame_10():
	if data.BG and host.AchSkin and not host.SKIN:
		...
		host.opponent.hitlag_ticks = 50
func _frame_33():
		if not data.shh and not host.LORDE:
			host.play_sound("InstallEnter")
		if not data.shh and host.LORDE:
			host.play_sound("LORDEInstallEnter")
		if not data.shh and not host.AchSkin and host.SKIN and not host.touhou and not data.Music:
			host.play_sound("SkinSFXLoop")
		if not data.shh and host.AchSkin and not host.SKIN or host.touhou and not data.Music:
			host.play_sound("AchSkinSFX")
		if data.Music and not host.AltMusic and not host.musicplaying:
			host.play_sound("InstallMusic")
			host.musicplaying = true
			print("normal")
		if data.Music and host.AltMusic and not host.musicplaying:
			host.play_sound("AltInstallMusic")
			host.musicplaying = true
			print("new")
		host.install = true
		if data.BG and not host.SKIN and not host.touhou and not host.LORDE:
			host.spawn_particle_effect(host.INSTALLBG, Vector2(0, 0))
		if data.BG and host.SKIN or host.touhou:
			host.spawn_particle_effect(host.INSTALLBGSPECIAL, Vector2(0, 0))
		if data.BG and host.AchSkin and not host.SKIN and not host.touhou:
			host.spawn_particle_effect(host.INSTALLBGACH, Vector2(0, 0))
		if data.BG and host.LORDE and not host.SKIN:
			host.spawn_particle_effect(host.INSTALLBGLORDE, Vector2(0, 0))
		host.charge += 1
