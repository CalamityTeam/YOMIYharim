...
func _frame_0():
	if host.SKIN or host.touhou and not host.AchSkin:
		timed_particle_scene = load ("res://Yharim/Characters/Yharim/Fx/GodRaysSpecial.tscn")
	if not host.AchSkin:
		host.charge += 1
...
func _frame_60():
	if host.AchSkin and not host.is_Script():
		host.charge += 1
